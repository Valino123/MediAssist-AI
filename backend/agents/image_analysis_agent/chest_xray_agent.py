...
class ChestXrayAgent(ImageClassifier):
    ...
    def load_model(self):
        ...
        We cache weights and processor files under models/chest_xray so that the
        first download is reused on subsequent runs.
        """
        try:
            logger.info(f"Loading model '{self.model_name}' from Hugging Face...")
            
            model_cache_dir = os.path.join(config.MODELS_PATH, "chest_xray")
